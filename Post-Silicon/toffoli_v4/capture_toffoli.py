# Toffoli Gate Leakage Capture Script
...
if __name__ == "__main__":
    # Parse arguments
    parser = argparse.ArgumentParser(description="Capture traces for Toffoli gate.")
    ...
    if not csv_spec.exists():
        logger.error("JSON definition does not exists")
        exit(1)
    ...
    logger.info(
        f"Configuring scope for {traces_num:,} traces of {sample_points:,} points."
    )
    ...
    try:
        ...
        logger.info(
            "Caught exception on reconnecting to target - attempting to reconnect to scope first."
        )
        logger.info(
            "This is a work-around when USB has died without Python knowing. Ignore errors above this line."
        )
        scope = cw.scope()
        target = cw.target(scope)
    ...
    if scope.get_name() == "ChipWhisperer Lite":
        ...
        try:
            ...
            scope.adc.stream_mode = eval(
                config.get("HUSKY", "stream_mode", fallback=False)
            )
        except ValueError:
            print(
                "check Husky clock freq ",
                scope.clock.clkgen_freq,
                " locked ",
                scope.clock.clkgen_locked,
            )
    ...
    try:
        with open(stimuli_file, newline="") as csvfile:
            ...
            for row in reader:
                input_str = (
                    row["a0"] + row["b0"] + row["c0"] +
                    row["a1"] + row["b1"] + row["c1"]
                ).lower()
                output_str = (row["x12"] + row["x15"]).lower()
    ...
    if len(pload_array) < traces_num:
        logger.error(
            f"Stimuli file size mismatch. Need {traces_num:,} vectors, found {len(pload_array):,}"
        )
        exit(1)
    elif len(pload_array) > traces_num:
        logger.warning(
            f"Stimuli file size mismatch. Using {traces_num:,} vectors from {len(pload_array):,}"
        )
    ...
    trace_map = np.memmap(
        trace_filename.as_posix(), dtype=trace_dtype, mode="w+", shape=trace_shape
    )
    ...
    logger.debug(target.get_simpleserial_commands())

    scope.errors.clear()

    for n in tqdm(range(traces_num)):

        logger.debug(f"Input: {pload_array[n]}; Length: {len(bytes.fromhex(pload_array[n]))}")
        ...
        while timed_out:
            ...
            if timed_out:= scope.capture(poll_done=True):
                logger.warning(f"ADC Errors: {scope.adc.errors}")
