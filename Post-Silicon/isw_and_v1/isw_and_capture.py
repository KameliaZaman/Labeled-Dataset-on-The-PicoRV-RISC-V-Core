...
# ISW AND Leakage Capture Script
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Capture traces for ISW AND.")
    ...
    use_int = args.type == "int"
    if args.debug:
        logger.setLevel(logging.DEBUG)

    folder_name = args.vectors if args.suffix is None else args.vectors + "_" + args.suffix
    ...
    try:
        ...
        logger.info("Caught exception on reconnecting to target - reconnecting.")
        scope = cw.scope()
        target = cw.target(scope)
    ...
    with open(stimuli_file, newline="") as csvfile:
        ...
        for row in reader:
            # Construct input: a0,a1,b0,b1,r
            input_str = (
                row["a0"] + row["a1"] + row["b0"] +
                row["b1"] + row["r"]
            ).lower()
            output_str = row["expected_and"].lower()
    ...
    trace_filename = traces_path.joinpath(f"{args.vectors}.traces_v1.npy")
