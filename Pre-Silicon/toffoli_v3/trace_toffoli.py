...
def read_metadata(dataframe, number_traces):
    """
    Parses the 'input' column of the dataframe and returns lists of a0, b0, c0, a1, b1, c1 hex strings.
    """
    a0_list = dataframe['a0'].tolist()[:number_traces]
    b0_list = dataframe['b0'].tolist()[:number_traces]
    c0_list = dataframe['c0'].tolist()[:number_traces]
    a1_list = dataframe['a1'].tolist()[:number_traces]
    b1_list = dataframe['b1'].tolist()[:number_traces]
    c1_list = dataframe['c1'].tolist()[:number_traces]

    return a0_list, b0_list, c0_list, a1_list, b1_list, c1_list
...
if __name__ == "__main__":
    ...
    random_data = 'tv_toffoli.csv' # CSV file with a0, b0, c0, a1, b1, c1
    ...
    a0_list, b0_list, c0_list, a1_list, b1_list, c1_list = read_metadata(data, number_of_traces)
    
    # ELF symbol loading
    elf = ELFFile(open(fname, "rb"))
    get_payload = get_symbol_by_name(elf, "toffoli_gate")
    ...
    for i in range(number_of_traces):
        ...
        ql.arch.regs.a1 = int(b0_list[i], 16)
        ql.arch.regs.a2 = int(c0_list[i], 16)
        ql.arch.regs.a3 = int(a1_list[i], 16)
        ql.arch.regs.a4 = int(b1_list[i], 16) 
        ql.arch.regs.a5 = int(c1_list[i], 16) 
